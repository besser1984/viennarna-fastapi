...
def evaluate_thermodynamics(sequence: str, temp_c: float, na_mM: float, mg_mM: float, dntp_mM: float):
    ...
    # Owczarzy (2008) free Mg2+ after dNTP chelation
    ...
    for offset in range(-(n - 1), n):
        ...
        # Track contiguous stacked duplexes
        i = 0
        while i < overlap_len - 1:
            ...
            if pair_key in NN_PARAMS:
                ...
                # Single initiation penalty per duplex block
                if sub1[i] in 'AT':
                ...
                # Apply Owczarzy salt correction factor on dS
...
def analyze_homodimer(req: HomodimerRequest):
    try:
        min_dg, alignment = evaluate_thermodynamics(
            req.sequence, req.temperature_c, req.na_mM, req.mg_mM, req.dntp_mM
        )

        warning = min_dg < -5.0 or (alignment and alignment["is_3prime_end"] and min_dg < -3.0)
